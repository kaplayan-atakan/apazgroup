...
async function testRedirect(source, expectedRedirect) {
  return new Promise((resolve) => {
    ...
    // Create request options to prevent automatic redirect following
    const options = {
      method: 'HEAD'
    };
...
async function testSitemap() {
  return new Promise((resolve) => {
    const req = http.request(`${baseUrl}/sitemap.xml`, { method: 'GET' }, (res) => {
...
async function testRobotsTxt() {
  return new Promise((resolve) => {
    const req = http.request(`${baseUrl}/robots.txt`, { method: 'GET' }, (res) => {
      const { statusCode } = res;
      let data = '';
